...
-- mySQL 은 DB와 관련된 명령을 실행할때
-- 가장먼저 어떤 DB를 사용할 것인지 알려줘야 한다

use myDB;
drop table tbl_test;
-- table 을 생성할때 컬럼의 이름은 사전에 있는 키워드는
-- 급적 사용하지 않는것이 좋다
-- 사전에 있는 키워드를 사용할때는 
...
MySQL을 사용하는데 
MySQL 에서 사용하는 방식이 DBMS 의 업계표준으로alter정착이
된 상태alter

대부분의 DBMS 들은 먼저 Database를 생성하고
사용자가 Database 에 접근할수 있는 권한을 부여하고
...
때문에 정수형 11자리 범위를 벗어나는 경우 데이터 insert 가 불가능한
문제가 있었다
5.7 버전 이후에는 BIGINT 형 타입이 도입되어 문제를 해결하였다
DB table을 만들때 AUTO_INCREMENT 칼럼에는 반드시 BIGINT형을 지정한다

MySQL 숫자형 데이터 타입
TINYINT : 1Byte : -128~127까지만 저장가능
SMALLINT : 2 Byte : -32768 ~23762 까지
MEDIUMIT : 3Byte : 0~ 16777215, -8388608 ~8388607 까지
INT :4Byte :-217483648 ~ 214783647 까지
BIGINT : 8Byte : -2의 7승 -1부터 2의 7승까지
*/

/*
AUTO_INCREMENT 칼럼을 만들때 주의해야할 점
데이터 타입은 BIGINT 로 설정
가급적 PRIMARY KEY로 설정할것
PK가 아닌경우 반드시 NOT NULL 과 UNIQUE 를 같이 설정한다
PK 또는 NOT NULL and UNIQUE 설정이 되지 않았을 경우
Table 생성에서 오류가 난다
*/
DROP TABLE  tbl_studnet;
...
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('s0101','이몽룡','컴공과',1,'010-111-1111','광주시');
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('s0102','홍길동','컴공과',3,'010-111-2222','광주시');
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('s0103','성춘향','법학과',2,'010-111-3333','광주시');
