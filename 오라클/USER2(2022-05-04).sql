...
평균 순으로 낮은 점수부터 높은 순으로 보이기
*/
SELECT sc_subject, SUM(sc_score) AS 총점, ROUND(AVG(sc_score),1) AS평균
FROM tbl_score
GROUP BY sc_subject
...
--과목별 총점과 평균을 계산하고 
--평균이 75점 이상인 과목만 보고 싶다
--통계함수로 계산된 결과에 대한 조건부여
SELECT sc_subject, SUM(sc_score) AS 총점, ROUND(AVG(sc_score), 2) AS평균
FROM tbl_score
GROUP BY sc_subject
...
GROUP BY sc_subject;
/*
통계함수와 GROUP BY 를 사용하여 통계연산을 수행할때 주의 사항
통계연산 결과에 어떤 조건을 부여하여 데이터를 보고자 할때는
WHERE절이나 HAVING 절에 조건을 부여 할수 있다.
