...
1. TableSpace 생성
    ...
    오라클 DBMS르 ㄹ사용하여 데이터를 관리하기 위해서는 
    제인먼저  TableSpace 를 생성해야 한다
    
    오라클 DBMS 를 설치하면 System 이 관리하는 TableSpace가 기본으로 생성된다
    System TableSpace 에는 중요한 데이터들이 저장되므로
    가급적 일반적인, 대향의 Date 를 저장하지 않는것이 좋다
...
/*
사용자를 생성하면서 DEFAULT TABLESPACE를 연결하지 않았을 경우
...
/*
사용자 삭제 명령문
DROP USER user1 CACADE;
*/
/*
오라클에서 새로운 사용자를 새엇ㅇ하면 생성된 사용자는 아무런 권한이 없다
심지어 DBMS에 접속할 수 있는 권한마저 없다
따라서 생성된 사용자에게는 적절한 수준의 권한을 부여야해야 한다
GRANT 부여, REVOKE 뺏기
*/
...
실무에서는 회사의 정책(Policy)에 따라 적절한 권한을 관리해야 하낟
권한이 잘못 부여되면 DB에 저장된 Data 에 손상을가할 수 있고
손상된 Data는 신뢰성을 잃게 된다

손상된 데이터가 저장된 데이터베이스 시스템은 '무결성이 파괴'되었다 라고 표현한다

"여기는 학습의 편의성을 위하여 사용자에게 최고권한을 부여 할 것이다"

DBA 권한은 SYSDBA  보다 약간의 제약이 있는 권한이다
오라클 DBMS의 중요한 정보 접근을 제외하고 모든 것을 다 할수 있다
심지어 다른 사용자의 TABLESPACE에도 접근할수 있다
