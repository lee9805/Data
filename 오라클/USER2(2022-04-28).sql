...
--생성된 테이브에 데이터를 추가하는 행위를 Data Create하고 한다
INSERT INTO tbl_student( st_num, st_name, st_tel, st_dept)
VALUES( '00001', '홍길동' , '010-0000-0000', '컴공과');

INSERT INTO tbl_student( st_num, st_name, st_tel, st_dept)
VALUES( '00002', '이몽룡' , '010-0000-0001', '전자과');

INSERT INTO tbl_student( st_num, st_name, st_tel, st_dept)
VALUES( '00003', '성춘향' , '010-0000-0002', '정보통신과');

--저장된 데이터의 조회(Query), 읽기
/*
SELETE * : 모든 칼럼을 다 표시하여 달라
SELETE st_num, st_name : 여러 칼럼중에서 st_num, st_name 칼럼만 표시해달라
...
SELECTION : 데이터를 조회할때 WHERE 조건절을 붙여 필요한 데이터 리스트만 보는 것

PROGECTION을 하면 실제 칼럼보다 적은 양의 칼럼을 보여준다
...
INSERT INTO tbl_student( st_num, st_name, st_tel, st_dept)
VALUES('00004','장영실','010-0000-0003','컴공과');
INSERT INTO tbl_student( st_num, st_name, st_tel, st_dept)
VALUES( '00005', '장보고' , '010-0000-0004', '컴공과');

--학과(st_dept)가 컴공과인 데이터를 SELECTION하여
--이름(st_name) 순으로 정렬하라
SELECT * 
FROM tbl_student
WHERE st_dept ='컴공과'
...
도구에서 데이터를 추가, 수정, 삭제명령을 수행한 경우
 ...
 이 상태에서 도구를 종료해 버리면
 Oracle DBMS 는 어떤 문제가 발생하여 종료된 석으로 생각하고
 데이터를 지워버린다
 도구를 사요앟여 접속하였을 경우는 도구를 종료하기 전에
...
VALUES('00006','신창원','010-0000-0005');
/*
...
사용자에게 권한을 부여( GRANT)하거나 회수(REVOKE)하는
...
CREATE TABLE tbl_score (
    st_stnum	VARCHAR2(5) PRIMARY KEY,
    sc_kor	NUMBER,	
    sc_eng	NUMBER,		
    sc_math	NUMBER,		
    sc_his	NUMBER,		
...
FROM tbl_student
GROUP BY st_dept
ORDER BY COUNT(st_dept) DESC, st_dept;

SELECT st_grade, COUNT(st_grade)
FROM tbl_student
GROUP BY st_grade
ORDER BY COUNT(st_grade) DESC;
